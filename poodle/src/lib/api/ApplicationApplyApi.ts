...
export const getFunc = async <ResponseType>(
    url:string, 
...
export const setFunc = async <RequestType>(url: string, payload: RequestType) => {
    const response = await client.patch(url, payload);
    return response;
}

export const typeStateToRequest = (state: RootState["ChoiceTypeState"]):userTypeServerType => {
    const {
        qualifacationExam,
        graduationStatus,
        graduationYear,
...
export const infoStateToRequest = (state: RootState["InfoState"]): userInfoServerType => ({
    name: state.name,
    sex: state.gender,
...
export const infoStateToGedRequest = (state: RootState["InfoState"]): gedInfoServerType => ({
...
export const infoResponseToState = (response: userInfoServerType): RootState["InfoState"] => ({
    name: response.name,
    gender: response.sex,
...
const infoStringToGradeNumber = (str: string) => {
    return str.split('')[0];
}
...
export const gradeResponseToState = (response: gradeServerType): RootState["GradeState"] => {
...
const stringArrayToGradeArray = (splitedStringArray: string[], subject: SubjectType) => {
  const buf = [];
...
export const gradeStateToRequest = (state: RootState["GradeState"]): gradeServerType => {
    return {
...
export const gradeStateToGedRequest = (state: RootState["GradeState"]): gedGradeServerType => ({
    ged_average_score: Number(state.score),
})

export const selfIntroductionStateToRequest = (state: RootState["IntroductionState"]): selfIntroductionServerType => {
    return {
        self_introduction: state.selfIntroduction
    }
};

export const studyPlanStateToRequest = (state: RootState["IntroductionState"]): studyPlanServerType => {
    return {
        study_plan: state.studyPlan,
    }
};

export const selfIntroductionResponseToState = (response: selfIntroductionServerType) => ({
    selfIntroduction: response.self_introduction,
});

export const studyPlanResponseToState = (response: studyPlanServerType) => ({
...
export const schoolStateToResponse = (state: any) => {
